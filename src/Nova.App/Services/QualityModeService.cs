...
/// model + params. Modes are defined as RedLeaf entities (type=quality-mode) and fetched at
/// startup; hardcoded fallbacks keep resolution working when RedLeaf is offline.
/// </summary>
public class QualityModeService
{
    ...
    public QualityModeService(NovaConfig config, LogService log, IHttpClientFactory httpFactory)
    {
        _config = config;
        _log = log;
    ...
    public async Task RefreshAsync(CancellationToken ct = default)
    {
        try
        {
            ...
            var url = $"{_config.Suite.RedLeaf.TrimEnd('/')}/api/entities?type=quality-mode&limit=100";
            var json = await http.GetStringAsync(url, ct);

            var parsed = ParseModes(json);
            if (parsed.Count == 0)
            {
                _log.Warn("quality-modes", "RedLeaf returned no usable quality-mode entities; keeping current modes");
                return;
            }
            ...
            _log.Info("quality-modes", $"Loaded {parsed.Count} quality mode(s) across {grouped.Count} tier(s) from RedLeaf");
        }
        catch (Exception ex)
        {
            _log.Warn("quality-modes", $"Failed to fetch quality modes from RedLeaf, using fallbacks: {ex.Message}");
        }
    }
    ...
    private static QualityMode? ParseOne(JsonElement item)
    {
        ...
        // The mode's settings live in the entity's JSONB `data` payload.
        var data = item;
